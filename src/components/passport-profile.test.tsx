...
/**
 * The passport's editable identity (docs/00 D30): shows the saved name/handle,
 * expands to a form, and persists edits so uploads pre-fill them. The invalid
 * handle gate mirrors the upload wizard so a broken handle can't be saved.
 */

function fakeApi(
  updateProfile: PassportBackend['updateProfile'] = vi
    .fn()
    .mockResolvedValue({ displayName: null, instagram: null }),
): PassportBackend {
...
describe('PassportProfile', () => {
  test('shows the saved name and handle, collapsed until edit is clicked', () => {
    render({ displayName: 'weekend warrior', instagram: 'neo_raver' })
  ...
  test('editing saves the typed values and reports back what the backend persisted', async () => {
    const user = userEvent.setup()
    // resolve values DISTINCT from the typed input so the onSaved assertion
    // proves the component forwards the backend's persisted result (trimmed /
    // nulled server-side), not the raw input it happened to send.
    const updateProfile = vi
      .fn()
      .mockResolvedValue({ displayName: 'raver (persisted)', instagram: 'raver_persisted' })
    ...
    // called with the typed input (@ stripped by the shared field)
    expect(updateProfile).toHaveBeenCalledWith({ displayName: 'raver', instagram: 'raver_01' })
  ...
  test('a broken handle blocks save — the gate matches the upload wizard', async () => {
    const user = userEvent.setup()
    const updateProfile = vi.fn().mockResolvedValue({ displayName: null, instagram: null })
