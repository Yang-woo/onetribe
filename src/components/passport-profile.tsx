...
/**
 * The passport's editable identity (docs/00 D30): the display name and
 * Instagram handle that uploads pre-fill. Collapsed to a quiet "@name · edit"
 * line; expands to a form. Saving persists to the profile (RLS owner-write) and
 * hands the new values back so the upload form pre-fills them next time.
 */
export function PassportProfile({
  ...
  api: PassportBackend
  onSaved: (next: { displayName: string | null; instagram: string | null }) => void
}) {
  ...
  const save = () => {
    ...
    setFailed(false)
    api
      .updateProfile({ displayName: name, instagram: ig })
      .then((saved) => {
        onSaved(saved)
        setEditing(false)
      })
